...
my $PDB25_list;
my $PDB90_list;
my $OUTFMT='summary,equivalences,transrot';
my $OUTHTML;
my $help;
my $clean;
my $MAXNPARA=99;
...
sub test_BLAST {
        ...
        my $method="WOLF"; if($nsse<$MINSSE) { if($DOSOAP>0) { $method="SOAP"; } else { return(); } }
...
sub generate_BLAST_list2 {
	...
	if($k>$K) { $k=$K; }
        foreach my $cd3 (@list3[0..$k]) {
                next if($seen{$cd3}>0);
                print OUT "$cd3\n";
		warn "# generate_BLAST_list2: $cd3 $n\n";
                $n++;
        }
        close(OUT);
        return($n);
...
sub generate_BLAST_list2_repset {
	...
	if($k>$K) { $k=$K; }
		...
		next if($seen{$cd3}>0);
                print OUT "$cd3\n";
		warn "# generate_BLAST_list2_repset: $cd3 $n\n";
                $n++;
        }
        close(OUT);
        return($n);
