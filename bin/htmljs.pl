...
if($#ARGV<0) { die "Usage $0 DALIDATDIR_1 DALIDATDIR_2 jobid TITLE [suppress_alignments]\n"; }
my($DALIDATDIR_1, $DALIDATDIR_2, $jobid, $TITLE, $suppress_ali)=@ARGV;
# query (cd1) in DALIDATDIR_1, targets (cd2) in DALIDATDIR_2
my $URL="/cgi-bin/daliviewer/readpdb.cgi";
my $gap='-';
my $identity_symbol='|';
my $lw=60;
my $MAXHITS=100;
...
while(<STDIN>) {
	if(/^-summar\s+\"(.*)\"/) { # this is last line in dbhit-block
		...
		if($cd2x !~ /^[mst]/) { $xhtml=~s/^(.{40})(.*)$/$1\<A HREF=\"$URL\?html=1&jobid=pdb&pdbid\=$cd2x&u\=$u&t\=$t\"\>PDB\<\/A\>$2/; $compnd=$2; }
		else { $xhtml=~s/^(.{40})(.*)$/$1\<A HREF=\"$URL\?jobid\=$jobid\&pdbid\=$cd2x&u\=$u&t\=$t\"\>PDB\<\/A\>$2/; $compnd=$2; }
		# patch compnd
		$compnd =~ s/MOLECULE: //;
		$compnd =~ s/ +$//;
		$compnd =~ s/ +/_/g;
		...
		$summary.=$x."\n";                          # STDERR — always include
		if ($nhit <= $MAXHITS) {
		    $summaryhtml.=$xhtml."\n";              # STDOUT — truncate at MAXHITS
		}
...
warn "$ranges\n";
warn "# Translation-rotation matrices\n";
warn "$matrices\n";
...
<input TYPE="button" NAME="D3" VALUE="Pfam" ONCLICK="return poppush();"/>
<input TYPE=\"reset\" VALUE=\"Reset Selection\"/>
<h2>Summary </h2>
The list is truncated at $MAXHITS hits.
<PRE>
    No:  Chain   Z    rmsd lali nres  \%id PDB  Description
$summaryhtml
</form>
...
function poppush() {
        document.optionform.action = "/cgi-bin/daliviewer/test_poppush.cgi";
        document.optionform.target = "_blank";
        document.optionform.submit();
}
