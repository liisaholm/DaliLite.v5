...
c!use fast indices!
       ...
       table(i+1,j+1)=scoretable(max(1,dist2(q)),max(1,dist(p)))
c				  t=t+table(i+1,j+1)
				end do
			  end do
c
...
c
99		if(iwhere+ir.le.exdim) ex(iwhere+ir)=x
	end do
...
c	 testcount=testcount+1
	 ...
	 if(aseg.eq.bseg) then
	  do i=1,ni(aseg)
        if(iwhere0+ci(i,aseg).gt.exdim) exit
		x=ex(iwhere0+ci(i,aseg))
		if(x.gt.est) est=x
	  end do
	 else
	  ...
	  do i=1,ni(aseg)
		iwhere=iwhere0+(ci(i,aseg)-1)*jw
		do j=1,ni(bseg)
                        if(iwhere+ci(j,bseg).gt.exdim) exit
			x=ex(iwhere+ci(j,bseg))
			if(x.gt.est) est=x
		end do
	  end do
