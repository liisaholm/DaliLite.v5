...
c
        ...
        character(len=5) s
        character(len=6) key
        integer, parameter :: MPI_CHARACTER_TAG=1, MPI_INTEGER_TAG=2
        integer, dimension(1) :: zero
...
c     $          line(1:6).eq.key)
        ...
        ! find idle slave
        call MPI_RECV(zero,0,MPI_INTEGER,MPI_ANY_SOURCE,MPI_INTEGER_TAG,
     $          MPI_COMM_WORLD,status,ierr)
        islave=status(MPI_SOURCE)
        ! send to slave
        zero(1)=len_trim(line)
        ! tag must be 1 for work unit; is 0 for shutdown
        call MPI_SEND(TRANSFER(s,(/0/)),5,MPI_CHARACTER,islave,
     $          MPI_CHARACTER_TAG,MPI_COMM_WORLD,ierr)
        call MPI_SEND(zero,1,MPI_INTEGER,islave,MPI_INTEGER_TAG,
     $          MPI_COMM_WORLD,ierr)
        call MPI_SEND(TRANSFER(line, (/0/)), n, MPI_CHARACTER, islave, 
     $          MPI_CHARACTER_TAG, MPI_COMM_WORLD, ierr)
c        call MPI_SEND(line,n,MPI_CHARACTER,islave,MPI_CHARACTER_TAG,
...
c
     ...
     $  lfirstonly,lfitz)
        ...
        zero(1)=0
        ! report for work
        call MPI_SEND(zero,0,MPI_INTEGER,0,MPI_INTEGER_TAG,
     $          MPI_COMM_WORLD,ierr)
        ...
        !do while(begintime-MPI_WTIME().lt.timeout)
          ! always receive cd1|SHUT!, check for tag
           call MPI_RECV(TRANSFER(cd1,(/0/)),5,MPI_CHARACTER,0,
     $          MPI_CHARACTER_TAG,MPI_COMM_WORLD,status,ierr)
          ...
          if(method(1:8).eq."FILTER95".or.method(1:2).eq."DP") then
                zero(1)=n
                call MPI_RECV(zero,1,MPI_INTEGER,0,
     $    MPI_INTEGER_TAG,MPI_COMM_WORLD,status,ierr)
                call MPI_RECV(TRANSFER(line,(/0/)),n,MPI_CHARACTER,0,
     $    MPI_CHARACTER_TAG,MPI_COMM_WORLD,status,ierr)
!                write(*,*) '# slave got bytes',n,line(1:n)
     ...
     $                  MPI_COMM_WORLD,ierr)
                ...
                call MPI_RECV(TRANSFER(cd2,(/0/)),5,MPI_CHARACTER,0,
     $                  MPI_CHARACTER_TAG,MPI_COMM_WORLD,status,ierr)
...
c
        ...
        do i=1,size-1
                ...
                call MPI_SEND(TRANSFER('SHUT!',(/0/)),5,MPI_CHARACTER,i,
     $                  MPI_CHARACTER_TAG,MPI_COMM_WORLD,ierr)
                ! wait for acknowledgement
                call MPI_RECV(zero,0,MPI_INTEGER,i,MPI_INTEGER_TAG,
     $                          MPI_COMM_WORLD,status,ierr)
...
c
     ...
     $          MPI_COMM_WORLD,status,ierr)
        ...
        call MPI_SEND(TRANSFER(cd1,(/0/)),5,MPI_CHARACTER,islave,
     $          MPI_CHARACTER_TAG,MPI_COMM_WORLD,ierr)
        call MPI_SEND(TRANSFER(cd2,(/0/)),5,MPI_CHARACTER,islave,
     $          MPI_CHARACTER_TAG,MPI_COMM_WORLD,ierr)
        !write(*,*) '# work unit ',cd1,' ',cd2,' ',islave,ierr
        if(npr.eq.0) return ! list1 x list2 methods
        ! DALICON, sends alignment
        pr(1)=npr
        call MPI_SEND(pr,1,MPI_INTEGER,islave,MPI_INTEGER_TAG,
     $          MPI_COMM_WORLD,ierr)
        call MPI_SEND(pr,4*npr,MPI_INTEGER,islave,MPI_INTEGER_TAG,
     $          MPI_COMM_WORLD,ierr)
