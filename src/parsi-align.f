...
c	write(*,*) 'output2',aseg,bseg,cut,iseg,jseg,iistart,ijstart,jjstart,jw
	...
	if(lseqtl.and.(trans(jres,jseg).lt.trans(ires,iseg))) goto 19
        if(iistart+ires.gt.exdim.or.iistart+ires.lt.1) goto 19
        if(jjstart+jres.gt.exdim.or.jjstart+jres.lt.1) goto 19
        if(iwhere+jres.gt.exdim) goto 19 ! HACK
...
c
	...
	logical ldb
c
	integer n,ir,ix,ali(maxseg),seglist(maxseg)
c
c	write(*,*) 'output1',iseg,nir,cut,idom,ali_laststart
        if(start(iseg,iseg).gt.0) then
          n=0
	  seglist(1)=iseg
          !write(*,*) 'iseg, start, nir',iseg,start(iseg,iseg),nir 
	  do ir=1,nir
                ix=start(iseg,iseg)+ir
                if(ix.lt.1.or.ix.gt.exdim) exit ! hack
...
c		write(*,*) 'ex',is,iseg,iwhere,ni(iseg),mi(iseg)
		do ir=1,min(exdim-iwhere,ni(iseg))
c			write(*,*) iseg,ir,irold(ir,iseg),trans1(ir,iseg),
c     $				ex(iwhere+irold(ir,iseg))
         if(iwhere+irold(ir,iseg).gt.exdim) exit ! HACK
         if(iwhere+irold(ir,iseg).lt.1) exit
...
c     $		(irold(jr,bseg),jr=1,ni(bseg))
				do jr=1,min(exdim-iwhere,ni(bseg))
        if(iwhereold+irold(jr,bseg).gt.exdim) exit ! HACK
...
c	write(*,*) 'leftshift',iseg,jseg,oldstart,laststart,mi(iseg),mi(jseg)
		if(iseg.eq.jseg) then
			...
			do ir=1,mi(iseg)
				i=i+1
        if(laststart+i.gt.exdim.or.oldstart+i.gt.exdim) exit ! HACK
