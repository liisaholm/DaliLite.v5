...
my $TSV_EXE="python3 $MPIDALI_BIN/parse_dali_txt_stream.py";
...
sub FSSP_output { # called from pairwise.pl, matrix.pl
	...
	foreach my $cd1 (@list) {
	        next if($cd1!~/\w{5}/);
	        my $cmd="$SORTDCCP_EXE < $cd1\.dccp | $FSSPFILTER_EXE $cd1 $zcut $nbest 1 | $FSSP_EXE $cd1 $dbsize $DALIDATDIR_1 $DALIDATDIR_2 | $FSSPSELECT_EXE $list2file $zcut $nbest | $HTML_EXE $DALIDATDIR_1 $DALIDATDIR_2 $JOBID \"$TITLE\" \"$OUTFMT\" $OUTHTML > $cd1\.html 2> $cd1\.txt; $TSV_EXE < $cd1\.txt > $cd1\.tsv ";
	        warn "$cmd\n";
	        system($cmd);
	}
}
...
sub generate_FSSP { # called from search.pl
        ...
        $_=`wc -l $pdblist`;
        my $dbsize=/(\d+)/;
        my $cmd="$SORTDCCP_EXE < $cd1\.dccp | $FSSPFILTER_EXE $cd1 $zcut $nbest 1 | $FSSP_EXE $cd1 $dbsize $DALIDATDIR_1 $DALIDATDIR_2 | $FSSPSELECT_EXE $pdblist $zcut $nbest | $HTML_EXE $DALIDATDIR_1 $DALIDATDIR_2 $JOBID \"$TITLE\" \"$OUTFMT\" $OUTHTML > $out_html 2> $out_txt; $TSV_EXE < $out_txt > $cd1\.tsv ";
        warn "$cmd\n";
        system($cmd);
}
